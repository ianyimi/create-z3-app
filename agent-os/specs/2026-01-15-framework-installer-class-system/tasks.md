...
- [x] 1.0 Complete type definitions module
...
- [x] 2.0 Complete OAuth provider registry module
  ...
  - [x] 2.12 Export helper function `getProvider(id: string): OAuthProvider | undefined`
...
- [x] 3.0 Complete string replacement utilities module
  - [x] 3.1 Implement `replacePlaceholder(filePath, placeholder, content): Promise<void>`
  ...
  - [x] 3.2 Implement `generateOAuthConfigBlock(providers: string[]): string`
  ...
  - [x] 3.3 Implement `generateOAuthUIConfigBlock(providers: string[]): string`
    - Generate better-auth-ui provider array
    - Format: `['google', 'github', ...]`
  - [x] 3.4 Implement `generateEnvVarsBlock(providers: string[]): string`
    - Generate environment variable declarations
    - Format: `{PROVIDER}_CLIENT_ID=\n{PROVIDER}_CLIENT_SECRET=`
  - [x] 3.5 Implement `detectIndentation(line: string): string`
...
- [x] 4.0 Complete FrameworkInstaller abstract base class
  - [x] 4.1 Define class constructor
    - Parameters: targetPath (string), projectName (string)
    - Store as protected class properties
  - [x] 4.2 Declare abstract method `updateOAuthConfig(selectedProviders: string[]): Promise<void>`
    - Updates Better Auth configuration file
    - Framework-specific file path
  - [x] 4.3 Declare abstract method `updateOAuthUIConfig(selectedProviders: string[]): Promise<void>`
    - Updates better-auth-ui configuration
    - Framework-specific file path
  - [x] 4.4 Declare abstract method `updateEnvExample(selectedProviders: string[]): Promise<void>`
    - Updates .env.example with OAuth variables
    - Framework-specific handling
  - [x] 4.5 Declare abstract method `applyTweakCNTheme(themeContent: string): Promise<void>`
    - Applies TweakCN CSS to Tailwind config
    - Framework-specific config file location
  - [x] 4.6 Declare abstract property `frameworkName: string`
    - Returns framework identifier for template selection
  - [x] 4.7 Implement concrete method `copyBaseFiles(): Promise<void>`
    - Use existing `copyTemplate()` from fileOperations.ts
    - Copy framework-specific template to targetPath
  - [x] 4.8 Implement concrete method `detectPackageManager(): PackageManager`
    - Check `npm_config_user_agent` environment variable
    - Parse for pnpm, yarn, bun, or default to npm
  - [x] 4.9 Implement concrete method `installDependencies(): Promise<void>`
...
- [x] 5.0 Complete TanStackInstaller class
  - [x] 5.1 Define class extending FrameworkInstaller
    - Call super() with targetPath and projectName
  - [x] 5.2 Define `frameworkName` property
    - Return: 'tanstack'
  - [x] 5.3 Implement `updateOAuthConfig(selectedProviders: string[]): Promise<void>`
  ...
  - [x] 5.4 Implement `updateOAuthUIConfig(selectedProviders: string[]): Promise<void>`
    - Target file: `src/lib/auth/client.ts`
    - Use `// {{OAUTH_UI_PROVIDERS}}` placeholder
    - Generate provider array for better-auth-ui
  - [x] 5.5 Implement `updateEnvExample(selectedProviders: string[]): Promise<void>`
  ...
  - [x] 5.6 Implement `applyTweakCNTheme(themeContent: string): Promise<void>`
...
- [x] 6.0 Complete NextJSInstaller class
  - [x] 6.1 Define class extending FrameworkInstaller
    - Call super() with targetPath and projectName
  - [x] 6.2 Define `frameworkName` property
    - Return: 'nextjs'
  - [x] 6.3 Implement `updateOAuthConfig(selectedProviders: string[]): Promise<void>`
    - Target file: `lib/auth.ts` or `lib/auth/index.ts`
    - Use `// {{OAUTH_PROVIDERS}}` placeholder
    - Generate socialProviders configuration block
    - Use replacePlaceholder utility
  - [x] 6.4 Implement `updateOAuthUIConfig(selectedProviders: string[]): Promise<void>`
    - Target file: `lib/auth-client.ts` or `lib/auth/client.ts`
    - Use `// {{OAUTH_UI_PROVIDERS}}` placeholder
    - Generate provider array for better-auth-ui
  - [x] 6.5 Implement `updateEnvExample(selectedProviders: string[]): Promise<void>`
    - Target file: `.env.example`
    - Use `# {{ENV_OAUTH_VARS}}` placeholder
    - Generate CLIENT_ID and CLIENT_SECRET for each provider
  - [x] 6.6 Implement `applyTweakCNTheme(themeContent: string): Promise<void>`
...
- [x] 7.0 Complete module exports and factory function
  ...
  - [x] 7.5 Implement factory function `createInstaller(framework, targetPath, projectName): FrameworkInstaller`
    - Return appropriate installer based on framework parameter
    - Throw error for unsupported framework
  - [x] 7.6 Export factory function as default or named export
